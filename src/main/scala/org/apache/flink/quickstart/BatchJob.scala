...
object BatchJob {
  ...
  def main(args: Array[String]) {
    ...
    val data: DataSet[String] = env.readTextFile("H:\\testFlink\\examples-scala-master\\examples-scala-master\\README.md");
    val dd: AggregateDataSet[(String, Int)] = data.flatMap(_.split(" ")).map((_,1)).groupBy(0).sum(1)
...
//    val wordcount = data.flatMap(_.split(" ")).map((_, 1)).groupBy(0).sum(1)
//    wordcount.print()
//    ds.print()
